...
public class ItemService {
    ...
    public ItemVo loadData(Long skuId) {
        ...
        CompletableFuture<Void> groupFuture = skuFuture.thenAcceptAsync(skuEntity -> {
            ResponseVo<List<ItemGroupVo>> groupResponseVo = this.pmsClient.queryGroupWithAttrValuesByCid(skuEntity.getCategoryId(), skuEntity.getSpuId(), skuId);
    ...
    public void generateHtml(ItemVo itemVo) {
        ...
        //初始化上下文对象，通过该对象给模板传递渲染所需压迫的数据
        Context context = new Context();
        context.setVariable("itemVo", itemVo);
        //初始化文件流：jdk1.8新语法
        try (PrintWriter printWriter = new PrintWriter("E:\\Java\\IntelliJ IDEA 2019.3.5_workspace\\html" + itemVo.getSkuId() + "html")) {
